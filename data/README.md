...
원천 데이터의 재배포 라이선스 제약과 GitHub 파일 용량 제한(100MB)에 따라, 데이터는 **GitHub 저장소**와 **AWS S3(객체 스토리지)**에 분산 배포한다.
...
| 출처 | 데이터 | 라이선스 | 배포 |
|------|--------|---------|------|
| **AI Hub** (반려동물 질병 데이터) | 수의사 Q&A 코퍼스 (원천·전처리), BERT 임베딩 | AI Hub 이용약관 — **재배포 제한** | S3 (출처 표기) |
