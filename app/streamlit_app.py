"""
반려견 증상 매칭 서비스 — Streamlit 앱
로컬: streamlit run app/streamlit_app.py
EC2:  systemd로 8501 포트 서비스 (docs/aws_migration.md Step 5 참고)
"""
import sys
from pathlib import Path

sys.path.append(str(Path(__file__).parent.parent))

import ast
import numpy as np
import pandas as pd
import streamlit as st
from sentence_transformers import SentenceTransformer
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.metrics.pairwise import cosine_similarity

from utils.config import DATA_PROCESSED, is_s3, S3_BUCKET, AWS_REGION

# ── 경로 설정 ─────────────────────────────────────────────────────────
PREPROCESSED_PATH = DATA_PROCESSED / "corpus_preprocessed.csv"
EMBED_PATH        = DATA_PROCESSED / "embeddings" / "db_embeddings.npy"
SBERT_MODEL       = "jhgan/ko-sroberta-multitask"

# AWS 마이그레이션 후 S3에서 임베딩 로드 (docs/aws_migration.md Step 4 참고)
# if is_s3():
#     import boto3, tempfile
#     s3 = boto3.client('s3', region_name=AWS_REGION)
#     with tempfile.NamedTemporaryFile(suffix='.npy') as tmp:
#         s3.download_file(S3_BUCKET, 'embeddings/db_embeddings.npy', tmp.name)
#         db_embeddings = np.load(tmp.name)


# ── 리소스 로드 (캐싱) ────────────────────────────────────────────────
@st.cache_resource(show_spinner="모델 로딩 중...")
def load_resources():
    df_db = pd.read_csv(PREPROCESSED_PATH)
    df_db = df_db[df_db["split"] == "train"].reset_index(drop=True)

    # TF-IDF 벡터화
    vectorizer   = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform(df_db["input_tokens"].fillna(""))

    # Sentence-BERT 임베딩 (사전 계산된 .npy 우선 사용)
    if EMBED_PATH.exists():
        db_embeddings = np.load(EMBED_PATH)
    else:
        model         = SentenceTransformer(SBERT_MODEL)
        db_embeddings = model.encode(
            df_db["input_normalized"].fillna("").tolist(),
            batch_size=64,
            normalize_embeddings=True,
            show_progress_bar=False,
        )
        EMBED_PATH.parent.mkdir(parents=True, exist_ok=True)
        np.save(EMBED_PATH, db_embeddings)

    sbert_model = SentenceTransformer(SBERT_MODEL)
    return df_db, vectorizer, tfidf_matrix, db_embeddings, sbert_model


# ── 매칭 함수 ─────────────────────────────────────────────────────────
def match_tfidf(query: str, vectorizer, tfidf_matrix, top_k: int = 5) -> np.ndarray:
    """TF-IDF 코사인 유사도 기반 상위 k개 인덱스 반환."""
    vec    = vectorizer.transform([query])
    scores = cosine_similarity(vec, tfidf_matrix).flatten()
    return scores.argsort()[::-1][:top_k], np.sort(scores)[::-1][:top_k]


def match_sbert(query: str, sbert_model, db_embeddings, top_k: int = 5):
    """Sentence-BERT 코사인 유사도 기반 상위 k개 인덱스 반환."""
    q_emb  = sbert_model.encode([query], normalize_embeddings=True)[0]
    scores = (db_embeddings @ q_emb).flatten()
    return scores.argsort()[::-1][:top_k], np.sort(scores)[::-1][:top_k]


# ── UI ────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="반려견 증상 매칭 서비스",
    page_icon="🐾",
    layout="wide",
)

st.title("🐾 반려견 증상 매칭 서비스")
st.caption("증상을 입력하면 유사한 수의사 Q&A를 찾아드립니다.")

# 사이드바 — 설정
with st.sidebar:
    st.header("⚙️ 설정")
    method = st.radio(
        "매칭 방법",
        ["Sentence-BERT (추천)", "TF-IDF"],
        index=0,
    )
    top_k = st.slider("결과 개수", min_value=1, max_value=5, value=3)
    lc_filter = st.selectbox(
        "생애주기 필터 (선택)",
        ["전체", "자견", "성견", "노령견"],
        index=0,
    )
    st.divider()
    st.caption("모델: jhgan/ko-sroberta-multitask")
    st.caption("데이터: AI Hub 반려견 질병 말뭉치")

# 리소스 로드
df_db, vectorizer, tfidf_matrix, db_embeddings, sbert_model = load_resources()

# 생애주기 필터 적용
if lc_filter != "전체":
    df_search    = df_db[df_db["lifeCycle"] == lc_filter].reset_index()
    search_embed = db_embeddings[df_search["index"].values]
    search_tfidf = tfidf_matrix[df_search["index"].values]
    search_df    = df_search
else:
    df_search    = df_db.copy()
    df_search["index"] = df_search.index
    search_embed = db_embeddings
    search_tfidf = tfidf_matrix
    search_df    = df_search

# 쿼리 입력
query = st.text_area(
    "증상을 입력하세요",
    placeholder="예: 강아지가 밥을 안 먹고 자꾸 토해요. 3일째 기운이 없어요.",
    height=100,
)

col1, col2 = st.columns([1, 5])
with col1:
    search_btn = st.button("🔍 검색", use_container_width=True, type="primary")

# ── 검색 실행 ─────────────────────────────────────────────────────────
if search_btn and query.strip():
    with st.spinner("유사 질문 검색 중..."):
        use_sbert = "BERT" in method
        if use_sbert:
            indices, scores = match_sbert(query, sbert_model, search_embed, top_k=top_k)
        else:
            indices, scores = match_tfidf(query, vectorizer, search_tfidf, top_k=top_k)

    st.subheader(f"📋 유사 질문 Top-{top_k} ({method.split()[0]})")

    for rank, (idx, score) in enumerate(zip(indices, scores), 1):
        row = search_df.iloc[idx]
        with st.expander(
            f"**{rank}위** | {row['lifeCycle']} · {row.get('department', '')} · {row.get('disease', '')}  |  유사도 {score:.3f}",
            expanded=(rank == 1),
        ):
            col_q, col_a = st.columns(2)
            with col_q:
                st.markdown("**질문**")
                st.write(row["input"])
            with col_a:
                st.markdown("**수의사 답변**")
                st.write(row["output"])

elif search_btn and not query.strip():
    st.warning("증상을 입력해주세요.")

# ── 푸터 ─────────────────────────────────────────────────────────────
st.divider()
st.caption("성균관대학교 빅데이터 대학원 · 데이터마이닝 과제 · 2025")
