...
| 소스 | 규모 | 역할 |
|------|------|------|
| AI Hub 반려견 질병 Q&A | 21,604건 | 수의사 답변 코퍼스 — 검색 대상·RAG 근거 |
| 네이버 지식iN 보호자 질문 | 114,439건 | intent 분류(증상 60,301·응급 27,408·처치 26,730) 학습·RAG 평가·커버리지 입력 |
| VeNom 국제 수의용어 | 39종 | 구어체→표준 증상어 정규화 34 + 모호 증상 되묻기 5 |

> AI Hub(질병)와 네이버(intent)는 라벨이 아닌 **질문 텍스트의 의미(BERT 임베딩)로 연결** — 보호자 질문의 **92.6%**를 수의사 코퍼스로 답변 가능(커버리지). 상세 명세: [`data/README.md`](data/README.md)
...
> McNemar test p = 0.61 · paired bootstrap 95% CI [-10, +20] %p (0 포함) — GT 50건의 +6%p 우위도 **통계적 유의성은 미확보**(n=50 소표본).
> BERT는 방향적 우위이며, 평가셋 확대 시 유의성 재검증이 필요.
...
인터랙티브 대시보드(React + Plotly)는 7개 탭으로 구성됩니다.

| 탭 | 내용 |
|----|------|
| **건강 상담** | **AI Hub 답변 검색** / **네이버 보호자 질문** 서브탭(동일 검색 UI) — 검색 2방향(TF-IDF vs BERT) + 그 아래 **4방향 답변 비교**(A.Gemini 단독·B.TF-IDF·C.BERT·D.RAG)에 관련성·근거성 지표 |
| 데이터 현황 | AI Hub·네이버·VeNom 3소스 규모·분포, 데이터 연결 흐름 |
| EDA | 생애주기·진료과 분포, intent × 증상 키워드 히트맵 |
| 모델 평가 | 검색(GT 50건·전체 2,399건 Hit@k·MAP·McNemar) / 답변(4방향 관련성·근거성) |
| 매칭 뷰어 | GT 50건 쿼리별 top-5 / 전체 2,399건 쿼리 데이터 테이블 |
| AI 채팅 | 4방향 라이브 비교(A.Gemini·B.TF-IDF·C.BERT·D.RAG) |
| 논의 & 한계 | 통계 유의성·추론비용·데이터 편향 등 8개 한계의 정직한 보정 |
...
> 예: "절뚝거려요"→**파행(lameness)**은 코퍼스에 114건 정규화 적용. 사전에 없는 변형 표현은 BERT 의미 임베딩이 보완하므로, 핵심 빈출 표현만 규칙화해도 구어체를 폭넓게 흡수합니다(규칙 수에 의존하지 않는 설계).
>
...
| 09 | `09_adult_dog_analysis.ipynb` | 성견 TF-IDF 우위 가설 검증 — 명사비율 자견 42.6·성견 43.3·노령 43.3%(차이 미미 → 소표본 편향) | — |
